...
// Actualizar la discapacidad si se envía el formulario de edición
if (isset($_POST["guardarCambios"])) {
    $id = $_POST['id'];
    $codigoDiscapacidad = $_POST['codigoDiscapacidad'];
    $nombreDiscapacidad = $_POST['nombreDiscapacidad'];

    // Consulta para actualizar los datos de la discapacidad
    $queryUpdate = "UPDATE discapacidad SET codigoDiscapacidad = '$codigoDiscapacidad', nombreDiscapacidad = '$nombreDiscapacidad' WHERE id = '$id'";
                ...
                <?php
                    ...
                    if ($resultSelect->num_rows > 0) {
                        while($row = $resultSelect->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td>" . $row['id'] . "</td>";
                            echo "<td>" . $row['codigoDiscapacidad'] . "</td>";
                            echo "<td>" . $row['nombreDiscapacidad'] . "</td>";
                            echo "<td>";
                            echo "<form method='post' action='' class='form-inline'>";
                            echo "<input type='hidden' name='id' value='" . $row['id'] . "'>";
                            echo "<input type='text' name='codigoDiscapacidad' value='" . $row['codigoDiscapacidad'] . "' class='form-control mb-2 mr-sm-2'>";
                            echo "<input type='text' name='nombreDiscapacidad' value='" . $row['nombreDiscapacidad'] . "' class='form-control mb-2 mr-sm-2'>";
                        ...
                        echo "<tr><td colspan='4'>No hay registros de discapacidades.</td></tr>";
                    }
