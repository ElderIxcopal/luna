...
if (isset($_POST['show_data'])) {
    $file = $_FILES['file_jefes']['tmp_name'];
...
if (isset($_POST['import_jefes'])) {
    $importData = $_SESSION['import_data'] ?? [];
    $success = true;

    foreach ($importData as $data) {
        $codigoJefe = $data['codigoJefe'];
        $nombreJefe = $data['nombreJefe'];
        $desde = $data['desde'];
        $hasta = $data['hasta'];

        $check_sql = "SELECT COUNT(*) FROM jefes WHERE codigoJefe = '$codigoJefe'";
        ...
        if ($exists) {
            ...
            $sql = "INSERT INTO jefes (codigoJefe, nombreJefe, desde, hasta) 
                    VALUES ('$codigoJefe', '$nombreJefe', " . ($desde ? "'$desde'" : "NULL") . ", " . ($hasta ? "'$hasta'" : "NULL") . ")";
        }
...
<div class="container mt-4">
    ...
    if (isset($_SESSION['import_data']) && !empty($_SESSION['import_data'])) {
        ...
        foreach ($_SESSION['import_data'] as $data) {
            echo '<tr>';
            echo "<td>{$data['codigoJefe']}</td>";
            echo "<td>{$data['nombreJefe']}</td>";
        ...
        <form method="POST" enctype="multipart/form-data">
            <button type="submit" name="import_jefes" class="btn btn-primary">Importar Jefes</button>
        </form>
        <?php
    } else {
        ?>
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                <label for="file_jefes">Selecciona un archivo Excel:</label>
                <input type="file" name="file_jefes" class="form-control-file" id="file_jefes">
            </div>
