...
if (isset($_POST['show_data'])) {
    $file = $_FILES['file_idiomas']['tmp_name'];
...
if (isset($_POST['import_idiomas'])) {
    ...
    foreach ($importData as $data) {
        ...
        $check_sql = "SELECT COUNT(*) FROM idiomas WHERE codigoIdioma = '$codigoIdioma'";
        ...
        if ($exists) {
            $sql = "UPDATE idiomas SET 
                    ...
                    WHERE codigoIdioma = '$codigoIdioma'";
        } else {
            $sql = "INSERT INTO idiomas (codigoIdioma, nombreIdioma, desde, hasta) 
                    VALUES ('$codigoIdioma', '$nombreIdioma', " . ($desde ? "'$desde'" : "NULL") . ", " . ($hasta ? "'$hasta'" : "NULL") . ")";
        }
...
<div class="container mt-4">
    ...
    if (isset($_SESSION['import_data']) && !empty($_SESSION['import_data'])) {
        ...
        echo '<thead>';
        echo '<tr>';
        echo '<th>Código de Idioma</th>';
        echo '<th>Nombre de Idioma</th>';
        echo '<th>Desde</th>';
        echo '<th>Hasta</th>';
        echo '</tr>';
        echo '</thead>';
        ...
        foreach ($_SESSION['import_data'] as $data) {
            echo '<tr>';
            echo "<td>{$data['codigoIdioma']}</td>";
            echo "<td>{$data['nombreIdioma']}</td>";
        ...
        <form method="POST" enctype="multipart/form-data">
            <button type="submit" name="import_idiomas" class="btn btn-primary">Importar Idiomas</button>
        </form>
        <?php
    } else {
        ?>
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                <label for="file_idiomas">Selecciona un archivo Excel:</label>
                <input type="file" name="file_idiomas" class="form-control-file" id="file_idiomas">
            </div>
